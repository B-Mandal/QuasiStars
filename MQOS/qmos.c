#include<stdio.h>                       /* MAXIMAL QUASI-OUT-STAR */
#include<stdlib.h>
#include<string.h>
#include<math.h>
#include<errno.h>
#define CBUF 100000
#define STR_IN "maximal_out_stars.txt"
...
int main(void)
{
          int order, i, vertex, deg, in_cnt, out_cnt, in_lim, size_set, max_size = 0 , temp_size;
          float gma, f;
          ...
          for(vertex=0; vertex<order; vertex++)             /// Out-Star Processing
          {
                    if(OUT_DEG[vertex] < 1)
                    {
                              if(IN_DEG[vertex] > 1)           /// Cannot be side vertex
                                        OUT_DEG[vertex] = -1;
                    ...
                    else
                    {
                              if(gma == 0.0)
                              {
                                        ...
                                        while(curr)
                                        {
                                                  if(OUT_DEG[curr->vertex - 1] == -1 || (IN_DEG[curr->vertex - 1]+OUT_DEG[curr->vertex - 1]) != 1)
                                                  {
                                                            curr = curr->next;
                                                            continue;
                                                  }
                              ...
                              else      /// gma not equal to 0.0
                              {
                                        ...
                                        while(curr)                   /// Out degree processing
                                        {
                                                  if(OUT_DEG[curr->vertex - 1] == -1 || (IN_DEG[curr->vertex - 1]+OUT_DEG[curr->vertex - 1]) != 1)
                                                  {
                                                            curr = curr->next;
                                        ...
                                        in_cnt = 0;
                                        o_curr = (struct node*)IN_LIST[vertex];
                                        o_strt  = o_curr;

                                        while(o_curr)                 /// in degree processing
                                        {
                                                  if(OUT_DEG[o_curr->vertex - 1] == -1 || (IN_DEG[o_curr->vertex - 1]+OUT_DEG[o_curr->vertex - 1]) != 1)
                                                  {
                                                            o_curr = o_curr->next;
                                                            continue;
                                                  }
                                                  ...
                                                  o_prev->next = now;
                                                  o_prev = now;
                                                  OUT_DEG[o_curr->vertex - 1] = -1;
                                                  o_curr = o_curr->next;
                                                  in_cnt++;
                                        }
                                                                                                              /// Quasi out-star processing

                                        f = (float)out_cnt / gma;
                                        in_lim = floor((double)f);
                                        in_lim = in_lim - out_cnt;             /// Permissible in degrees
                                        ...
                                        else
                                        {
                                                  ...
                                                  strcpy(temp, "");
                                                  strcpy(fname, "d");
                                                  sprintf(temp, "%d%d.txt", in_cnt, in_lim);
                                                  strcat(fname, temp);
                                                  ...
                                                  {
                                                            fclose(dict);
                                                            genComb(in_cnt, in_lim);
                                                  }


                                                  readCombNWriteSet(fname, i_strt, o_strt, prev, in_cnt, in_lim);
          ...
          fprintf(finfo, "\nGamma: %0.2f", gma);
          fprintf(finfo, "\nNo of out-star found: %d", ns);
          fprintf(finfo, "\nMax  out-star size: %d", max_size);
          fclose(finfo);
...
void getGraph(int order, int *arr_deg, int **arr_list, char *fname)             /// OLD
{
          FILE *fp;
          char buf[CBUF], *ltok;
          int i = 0, deg, vertex;
...
void writeSet(struct node *strt)                  /// Writing Set
{
...
void freeSet(struct node *f)            /// Freeing memory.
{
          struct node *temp;
          while(f)
          {
...
int getSetSize(struct node *x)          /// Calculate Set Size
{
          int i=0;
          while(x)
          {
...
/* NEW Function definition for processing Graph*/
